...
- The replacement font comes from the font program pdf.js rebuilds from the embedded font (the same outlines the viewer shows). Fonts pdf.js cannot hand over (Type 3, some non-embedded standard fonts) fall back to the closest Standard-14 family. Characters that are not part of a subsetted font are drawn with the fallback fonts below (in their sans-serif face), and only become `?` when no fallback has them either.
...
- Text you add is exported with the Standard-14 fonts (Helvetica/Times/Courier). Characters outside WinAnsi are drawn with embedded fallback fonts: Liberation Sans for Latin, Greek and Cyrillic, and Noto Sans SC (regular weight only, fetched from the app's own origin in small chunks only when needed) for Chinese and Japanese. Emoji and scripts neither font covers (Arabic, Hebrew, Devanagari, Hangul syllables, …) are still replaced with `?`. Existing text rendering supports embedded fonts and CJK via pdf.js CMaps.
