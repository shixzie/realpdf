...
describe('local library', () => {
  ...
  it('saves, renames, restores and rebuilds a document', async () => {
    const { page, pageErrors } = app

    await gotoHome(page)
    await openPdf(page, SAMPLE_PDF, { settle: 1000 })
    ...
    const rebuiltPath = path.join(OUT_DIR, 'library-rebuilt.pdf')
