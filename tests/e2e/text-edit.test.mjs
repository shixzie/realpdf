...
describe('text editing', () => {
  ...
  it('deletes the original glyphs and embeds the replacement', async () => {
    ...
    await page.keyboard.press('Escape')
    await page.waitForTimeout(500)
    const wrappedTop = await pixelStats(page, { x: 56, y: 162, w: 110, h: 22 })
    const wrappedBottom = await pixelStats(page, { x: 56, y: 186, w: 110, h: 22 })
    ...
    const secondLine = await exportedPixelStats(context, exported, { x: 56, y: 162, w: 110, h: 46 })
